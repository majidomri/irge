...
const Masonry: React.FC<MasonryProps> = ({
  ...
  const grid = useMemo<GridItem[]>(() => {
    ...
    const columnWidth = (width - totalGaps) / cols;
  ...
  const handleMouseEnter = (id: string) => {
    if (scaleOnHover) gsap.to(`[data-masonry-key="${id}"]`, { scale: hoverScale, duration: 0.3, ease: 'power2.out' });
  };

  const handleMouseLeave = (id: string, el: HTMLElement) => {
    if (scaleOnHover) gsap.to(`[data-masonry-key="${id}"]`, { scale: 1, duration: 0.3, ease: 'power2.out' });
  ...
  return (
    <div ref={containerRef} className="relative w-full" style={{ height: totalHeight }}>
      {grid.map(item => (
        ...
        >
          {item.content ? (
            <div className="relative w-full h-full overflow-hidden rounded-[10px]">{item.content}</div>
          ) : item.img ? (
            <div
