    ...
    <div className="fixed inset-0 z-200 flex items-end sm:items-center justify-center">
      ...
      <section className="relative w-full sm:max-w-lg rounded-t-3xl sm:rounded-3xl overflow-hidden"
        ...
        <div className="p-5">
          ...
          {/* Full biodata — Raw or Structured view */}
          {bioView === 'structured' ? (
...
// ── Biodata schema renderer (json-render pattern, homegrown) ─────────────────
//
// Schema: { sections: [{ heading, rows: [{ label, value }] }], about? }
// parseBiodata() extracts structured fields from freeform text.
// BiodataStructured renders the schema as a card — no AI required.
...
function parseBiodata(title: string, body: string): BioSchema {
  ...
  // Title often contains summary info — add as first row if not duplicated
