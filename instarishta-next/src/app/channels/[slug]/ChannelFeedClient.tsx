...
/**
 * The feed, hydrated from what the server already rendered.
 *
 * Every piece of the first screen arrives as a prop: the channel, page 0 of
 * its posts, the total and the sibling strip. Nothing is fetched on mount any
 * more, which is the whole point -- the tiles are in the HTML, so the browser
 * can start the first image while the JS bundle is still parsing instead of
 * six seconds after it finishes.
 *
 * The props are the initial state and nothing more: from page 1 on, this
 * component fetches for itself exactly as before.
 */
export default function ChannelFeedClient({
  slug,
  initialChannel,
  initialPosts,
  initialTotal,
  ...
  initialPosts: IPost[];
  initialTotal: number;
  initialSiblings: { id: string; name: string; slug: string }[];
}) {
  const [channel,  setChannel]  = useState<IChannel | null>(initialChannel);
  const [posts,    setPosts]     = useState<IPost[]>(initialPosts);
  const [loading,  setLoading]   = useState(false);
  const [error,    setError]     = useState('');
  // Page 0 is already on screen, so the next fetch is page 1.
  const [page,     setPage]      = useState(1);
  // A first page that came back short IS the whole channel; without this the
  // scroll sentinel would ask for a second page that cannot exist.
  const [done,     setDone]      = useState(initialPosts.length < POST_PAGE_SIZE);
  const [total,    setTotal]     = useState<number | null>(initialTotal);
  const [siblings, setSiblings]  = useState<{ id: string; name: string; slug: string }[]>(initialSiblings);
  ...
  useEffect(() => {
    /**
     * Only the live subscription is left here.
     *
     * The channel, its first page, the count and the sibling strip all
     * arrived as props from the server, so fetching them again on mount would
     * be a second round trip for data already on the screen -- and would
     * briefly replace it while in flight.
     */
    realtimeRef.current = subscribeChannel(initialChannel.id, (post) => {
