...
export default function GradientText({
  children,
  className = '',
  colors = ['#00C87A', '#ffffff', '#00754A', '#ffffff'],
  animationSpeed = 6,
  showBorder = false,
  ...
  useEffect(() => { elapsedRef.current = 0; progress.set(0); }, [animationSpeed, yoyo]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <motion.span
      className={`relative inline-flex items-center justify-center overflow-hidden ${showBorder ? 'py-1 px-2 rounded-[1.25rem]' : ''} ${className}`}
    ...
    >
      {showBorder && (
        ...
        >
          <div className="absolute rounded-[1.25rem]"
            style={{ width: 'calc(100% - 2px)', height: 'calc(100% - 2px)', left: '50%', top: '50%', transform: 'translate(-50%, -50%)', background: '#1E3932' }} />
        </motion.div>
      )}
      <motion.span
